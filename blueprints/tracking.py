...
def api_position_latest():
    ...
    if source:
        row = conn.execute(
            "SELECT timestamp,latitude,longitude,speed_knots,course,heading,nav_status,source FROM positions WHERE user_id=? AND source=? ORDER BY timestamp DESC LIMIT 1",
            (uid, source)).fetchone()
    else:
        row = conn.execute(
            "SELECT timestamp,latitude,longitude,speed_knots,course,heading,nav_status,source FROM positions WHERE user_id=? ORDER BY timestamp DESC LIMIT 1",
            (uid,)).fetchone()
    conn.close()
    if row is None:
...
def api_position_track():
    ...
    if source:
        rows = conn.execute(
            "SELECT timestamp,latitude,longitude,speed_knots,course,source FROM positions WHERE user_id=? AND timestamp>=datetime('now',? || ' hours') AND source=? ORDER BY timestamp ASC",
            (uid, f"-{hours}", source)).fetchall()
    else:
        rows = conn.execute(
            "SELECT timestamp,latitude,longitude,speed_knots,course,COALESCE(source,'ais') as source FROM positions WHERE user_id=? AND timestamp>=datetime('now',? || ' hours') ORDER BY timestamp ASC",
            (uid, f"-{hours}",)).fetchall()
    conn.close()
...
def api_status():
    ...
    ais_row = conn.execute(
        "SELECT timestamp,latitude,longitude FROM positions WHERE user_id=? AND (source='ais' OR source IS NULL) ORDER BY timestamp DESC LIMIT 1",
        (uid,)).fetchone()
    inreach_row = conn.execute(
        "SELECT timestamp,latitude,longitude FROM positions WHERE user_id=? AND source='inreach' ORDER BY timestamp DESC LIMIT 1",
        (uid,)).fetchone()
